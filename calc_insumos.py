...
if 'quantidade_valor' not in st.session_state:
    st.session_state['quantidade_valor'] = 0.0
if 'preco_valor' not in st.session_state:
    st.session_state['preco_valor'] = 0.0

# Função para resetar o formulário (SOLUÇÃO DEFINITIVA)
def resetar_formulario():
    ...
    st.session_state['quantidade_valor'] = 0.0
    st.session_state['preco_valor'] = 0.0
    # Incrementa contador para gerar novas chaves de widgets
...
if st.session_state['capturar']:
    ...
    if foto is not None:
        ...
        if foto_hash != st.session_state['foto_hash']:
            ...
            st.session_state['quantidade_valor'] = 0.0
            st.session_state['preco_valor'] = campos['preco'] if campos['preco'] else 0.0
            st.rerun()
        else:
            st.session_state['foto_bytes'] = foto_bytes
            st.session_state['capturar'] = False
...
# Formulário único para edição dos dados (USANDO CHAVES DINÂMICAS)
st.subheader("Dados do Produto")
col1, col2 = st.columns([2, 2])

# Gera chaves únicas para os widgets baseadas no contador
...
with col1:
    ...
    quantidade = st.number_input(
        ...
        step=0.01,
        format="%.2f",
        value=st.session_state['quantidade_valor'],
        key=f"quantidade_input_{counter}"
    )
    ...
    preco = st.number_input(
        ...
        step=0.01,
        format="%.2f",
        value=st.session_state['preco_valor'],
        key=f"preco_input_{counter}"
    )
...
st.session_state['quantidade_valor'] = quantidade
st.session_state['preco_valor'] = preco
